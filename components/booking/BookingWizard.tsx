...
export default function BookingWizard() {
  ...
  return (
    <div className="bg-white rounded-3xl border border-border-soft p-8 md:p-10 shadow-sm max-w-3xl mx-auto">
      {/* STEPS INDICATOR */}
      <div className="flex justify-between border-b border-border-soft pb-6 mb-8">
        <div
          className={`text-sm ${step >= 1 ? 'text-primary-500 font-bold' : 'text-charcoal-300'}`}
        >
          1. {locale === 'fr' ? 'Date' : 'Date'}
        </div>
        <div
          className={`text-sm ${step >= 2 ? 'text-primary-500 font-bold' : 'text-charcoal-300'}`}
        >
          2. {locale === 'fr' ? 'Informations' : 'Details'}
        </div>
        <div
          className={`text-sm ${step >= 3 ? 'text-primary-500 font-bold' : 'text-charcoal-300'}`}
        >
          3. {locale === 'fr' ? 'Paiement' : 'Payment'}
        </div>
      </div>
      ...
      {step === 3 && selectedDate && (
        <div className="space-y-6">
          ...
          <div className="p-6 bg-sand-50 rounded-2xl border border-border-soft space-y-4">
            ...
            <div className="flex justify-between text-charcoal-500">
              <span>
                {passengers} {locale === 'fr' ? 'voyageur(s)' : 'traveler(s)'} x{' '}
                {selectedDate.price} €
              </span>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 pt-4">
            ...
            <div className="flex flex-col md:flex-row gap-3 w-full md:w-auto">
              ...
              <button
                ...
                disabled={checkoutLoading}
                className="px-8 py-3 bg-primary-400 hover:bg-primary-500 text-white rounded-full font-medium transition-colors"
              >
                {checkoutLoading
                  ? locale === 'fr'
                    ? 'Redirection...'
